...
def main():
    ...
    use_epochs_pretrain = epochs_pretrain['tDNN']
    ...
    model_emb = models.get_baseline_nn_model(num_gene_features = num_gene_features,
        vocab_size = vocab_size,
        drug_len = drug_len,
        embed_dim = embed_dim,
        drug_filters = drug_filters,
    ...
    predictions = model.predict([gene_data,drug_data],batch_size = batch_size)
    ...
    for i in tqdm(np.arange(gene_data.shape[0])):
        cur_data = [gene_data[i:i+1],drug_data[i:i+1]] 
        gene_importances_scaled, drug_importances_scaled = ig.get_gene_drug_importances_for_instance(cur_data, model_con,
                                             model_emb, alphas)
